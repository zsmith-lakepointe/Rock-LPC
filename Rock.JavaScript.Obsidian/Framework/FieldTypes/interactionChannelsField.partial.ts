...
const editComponent = defineAsyncComponent(async () => {
    return (await import("./interactionChannelsFieldComponents")).EditComponent;
});

// The configuration component can be quite large, so load it only as needed.
const configurationComponent = defineAsyncComponent(async () => {
    return (await import("./interactionChannelsFieldComponents")).ConfigurationComponent;
});

export class InteractionChannelsFieldType extends FieldTypeBase {
