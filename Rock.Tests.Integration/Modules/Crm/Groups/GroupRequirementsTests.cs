...
namespace Rock.Tests.Integration.Crm.Groups
{
    ...
    public class GroupRequirementsTests
    {
        ...
        public void CalculateGroupRequirementsJob_CanProcess()
        {
            ...
            personIdList = allPeopleIdList.GetRandomizedList( 1000 );
            CreateTestGroupWithRequirement( $"Test Group 1", Guid.NewGuid(), personIdList );
            personIdList = allPeopleIdList.GetRandomizedList( 1000 );
            CreateTestGroupWithRequirement( $"Test Group 2", Guid.NewGuid(), personIdList );
            personIdList = allPeopleIdList.GetRandomizedList( 1000 );
            CreateTestGroupWithRequirement( $"Test Group 3", Guid.NewGuid(), personIdList );

            TestHelper.StartTimer( "Calculate Group Requirements" );
